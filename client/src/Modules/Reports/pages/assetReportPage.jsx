// Place this file at: client/src/Modules/Reports/pages/assetReportPage.jsx
...
import AssetReportTable, { buildColumns, exportValue } from '../assetReportTable';
...
const emptyFilters = { assetGroup: [], category: [], itemClass: [], activeOnly: 'all', status: [], acquiredFrom: '', acquiredTo: '', amountFrom: '', amountTo: '', writtenOff: 'all' };
const emptyIncludes = { department: true, location: true, holder: false, brand: false, serialNumber: false, writtenOff: false };

export default function AssetReportPage({ setHeaderTitle }) {
  ...
  useEffect(() => { setHeaderTitle?.('Asset Report'); }, [setHeaderTitle]);
  ...
  const buildParams = (nextPage, nextPageSize) => ({
    ...
    status: filters.status.join(','), activeOnly: filters.activeOnly === 'active',
    writtenOff: filters.writtenOff === 'all' ? undefined : filters.writtenOff === 'yes',
  ...
  const fetchReport = async (nextPage = page, nextPageSize = pageSize) => {
    setLoading(true);
    try {
      const response = await api.get('/assetReport', { params: buildParams(nextPage, nextPageSize) });
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      while (true) {
        const response = await api.get('/assetReport', { params: buildParams(currentPage, EXPORT_PAGE_SIZE) });
      ...
      link.href = url;
      link.setAttribute('download', `AssetReport_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return <div className="w-full">
    ...
    <AssetReportTable rows={rows} total={total} page={page} pageSize={pageSize} loading={loading} includes={includes} onPageChange={(next) => { setPage(next); fetchReport(next); }} onPageSizeChange={(next) => { setPageSize(next); setPage(0); fetchReport(0, next); }} />
  </div>;
}