...
import ADLineItems from './adLinesItems';
...
export default function DisposalTabs({
    useProps,
    isCreating,
    isEditing,
    rows,
    ...
    <Box sx={{ bgcolor: 'background.paper', width: '100%',  borderRadius: 1, boxShadow: 3, paddingBottom: 2 }}>
      <AppBar position="static" sx ={{  borderRadius: 2, boxShadow: 3 }}>
        ...
        >
          ...
          {(viewApproval && !state.isCreating && copyDocNo?.length > 0 && currentHeader?.xpost !== 0 && currentHeader?.xpost !== 3) &&
      ...
      <TabPanel value={value} index={1} dir={theme.direction}>
        {(viewApproval && !state.isCreating && currentHeader?.xpost !== 0 && currentHeader?.xpost !== 3) && (
