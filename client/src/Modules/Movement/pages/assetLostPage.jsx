...
// Components
import AssetLostTabs from '../assetlost/AssetLostTabs';
...
import { useALData } from '../../../hooks/useAssetLost_reducer';
...
export default function ALFormPage(useProps) {
  ...
  const loadALData = useCallback(async () => {
    ...
    await getALData(copyDocNo);
  }, [copyDocNo, getALData]);

  useEffect(() => {
    loadALData();
  }, [copyDocNo, loadALData]);

const refreshData = useCallback(async () => {
  if (!copyDocNo) return;
   
  // Reload the data
  await forceRefreshAL(copyDocNo);
  // Increment refresh trigger to force re-renders of child components if needed
  setRefreshTrigger(prev => prev + 1);
}, [copyDocNo, getALData, forceRefreshAL]);
  ...
  const baseHeader = state.selectedAL;
  const currentHeader = state.isCreating || state.isEditing
    ? state.createALHeader
    : state.selectedAL;
  const currentALItems = state.isCreating || state.isEditing
    ? state.createALDetails
    : state.assetLostDetails;
...
const handleEmployeeChange = (event, newValue) => {
  ...
  const handleSave = async () => {
    ...
    if (!state.createALHeader?.EmpID) {
      alert('Please enter employee.');
      return;
    }
    if (!state.createALDetails || state.createALDetails.length === 0) {
      alert('Please add at least one item');
      return;
    }
    const invalidRows = state.createALDetails.filter(row => !row.ItemNo);
  ...
  const handleConfirmSave = async () => {
    const newAANo = state.createALHeader?.AAFNo;
  ...
  const handleConfirmPost = async () => {
    ...
    const result = await postAssetLost(docNo);
    if (result.success) {
        ...
        <Box className='flex flex-wrap justify-between w-full h-full gap-1'>
          ...
          <div className='flex gap-2'>
            ...
            <span className='text-xs text-gray-500'>
              {assetLostHeaders && assetLostHeaders.length > 0 
                ? (() => {
                    const latest = [...assetLostHeaders].sort((a, b) => {
                      const numA = parseInt(String(a.AAFNo).split('-').pop() || 0);
                      const numB = parseInt(String(b.AAFNo).split('-').pop() || 0);
                      return numB - numA;
                    })[0];
          ...
          <Box className='mt-2'>
            ...
            <div className='flex flex-wrap items-start justify-start w-full gap-10 mt-4'>
                ...
                <textarea
                  ...
                  minRows={2}
                  value={currentHeader?.xREMARKS || ''}
                  onChange={(e) => handleHeaderChange('xREMARKS', e.target.value)}
