...
// Components
import DisposalTabs from '../disposal/disposalTabs';
...
//   const { adHeaders } = useAD_h();
  ...
  const loadADData = useCallback(async () => {
    ...
    await getADData(copyDocNo);
  }, [copyDocNo, getADData]);

  useEffect(() => {
    loadADData();
  }, [copyDocNo, loadADData]);

const refreshData = useCallback(async () => {
  if (!copyDocNo) return;
   
  // Reload the data
  await forceRefreshAD(copyDocNo);
  // Increment refresh trigger to force re-renders of child components if needed
  setRefreshTrigger(prev => prev + 1);
}, [copyDocNo, getADData, forceRefreshAD]);
  ...
  const baseHeader = state.selectedAD;
  const currentHeader = state.isCreating || state.isEditing
    ? state.createADHeader
    : state.selectedAD;
  const currentADItems = state.isCreating || state.isEditing
    ? state.createADDetails
    : state.adDetails;
  ...
  const handleSave = async () => {
    ...
    if (!state.createADHeader?.Remarks) {
      alert('Please enter remarks.');
      return;
    }
    if (!state.createADDetails || state.createADDetails.length === 0) {
      alert('Please add at least one item');
      return;
    }
    const invalidRows = state.createADDetails.filter(row => !row.FAC_NO);
  ...
  const handleSuccessfulCreate = useCallback(async (newADNo) => {
    await refreshCompanyConfig();
    setSearchParams({ docId: newADNo });
  ...
  const handleConfirmSave = async () => {
    ...
    await confirmSave();
    if (wasCreating && newADNo) {
      handleSuccessfulCreate(newADNo);
    }
  };
  ...
  const handleConfirmPost = async () => {
    closePostDialog();
    const docNo = copyDocNo || baseHeader?.AD_No;
    if (!docNo) {
    ...
    const result = await postDisposal(docNo);
    if (result.success) {
          ...
          <div className='flex flex-wrap gap-2'>
            ...
            <span className='text-xs text-gray-500'>
              {adHeaders && adHeaders.length > 0 
                ? [...adHeaders].sort((a, b) => {
                    const numA = parseInt(String(a.AD_No).split('-').pop() || 0);
                    const numB = parseInt(String(b.AD_No).split('-').pop() || 0);
                    return numB - numA;
                  })[0]?.AD_No 
                : '---'}
            </span>
            ...
            <span className='text-xs text-gray-500'>
              {adHeaders && adHeaders.length > 0 
                ? (() => {
                    const latest = [...adHeaders].sort((a, b) => {
                      const numA = parseInt(String(a.AD_No).split('-').pop() || 0);
                      const numB = parseInt(String(b.AD_No).split('-').pop() || 0);
                      return numB - numA;
                    })[0];
          ...
          <Box className='mt-2'>
            <div className='flex flex-wrap items-center justify-start w-full gap-10 mt-4'>
              ...
              <Autocomplete
                ...
                size='small'
                options={departments} 
                value={currentHeader?.Department_Code || ''}
              ...
              <label className='text-base font-normal text-gray-500 w-28'>Evaluated by : </label>
              <input
                type="text"
                disabled
                className="text-base font-semibold text-gray-500"
                value={currentHeader?.Evaluated_By || userName}
