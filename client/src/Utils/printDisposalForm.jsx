// Utils/printDisposalForm.js
//
// Builds a professional, print-ready Disposal Form (AD) document and opens
// it in a new browser tab/window so the user can preview and print (or
// "Save as PDF") it via the native browser print dialog.
//
// Mirrors Utils/printJobOrder.js and Utils/printTransferForm.js so all
// three documents share the same letterhead, company info, and overall
// look & feel.
//
// Usage (see disposalPage.jsx):
//
//   import { openADPrintPreview } from '../../../Utils/printDisposalForm';
//
//   <CustomBtn variant='printBtn' iconType='print' onClick={() => openADPrintPreview({
//     currentHeader,
//     currentADItems,
//     docStatus,
//     companyConfig,
//     userName,
//     totalLevels,
//     approvalLogs,      // optional - filtered approval log rows for this AD
//   })}>
//     Preview
//   </CustomBtn>
...
// Same numeric-with-commas formatting as Utils/formatWithCommas.js, used
// for the Salvage Amount column.
...
// Mirrors the docStatus() mapping used in disposalPage.jsx, with a
...
function buildItemsRows(items) {
  if (!items || items.length === 0) {
    return `
      <tr>
        <td colspan="9" class="empty-row">No line items on this Disposal Form.</td>
      </tr>`;
  }

  return items
    .map((row, idx) => {
      const disposalType = row.Disposal_Type || '—';
      const salvage = disposalType === 'Sell' ? formatAmount(row.salvage_amount) : '—';
      return `
        <tr>
          ...
          <td class="center">
            <span class="type-pill type-${disposalType === 'Sell' ? 'sell' : 'dispose'}">${escapeHtml(disposalType)}</span>
          </td>
          <td class="amount-cell">${salvage}</td>
...
function buildTotalsRow(items) {
  const total = (items || [])
    .filter((r) => r.Disposal_Type === 'Sell')
    .reduce((sum, r) => sum + (Number(r.salvage_amount) || 0), 0);

  return `
    <tr class="totals-row">
      <td colspan="7" class="totals-label">Total Estimated Salvage Value</td>
      <td class="amount-cell totals-value" colspan="3">${formatAmount(total)}</td>
    </tr>`;
}
...
export function generateADPrintHTML({
  currentHeader = {},
  currentADItems = [],
  ...
  // — kept identical to printJobOrder.js / printTransferForm.js so all
  // documents share letterhead info.
  ...
  const adNo = currentHeader?.AD_No || '';
  ...
  const evaluatedBy = currentHeader?.Evaluated_By || userName || '—';
  const remarks = currentHeader?.Remarks || '';

  const itemsRows = buildItemsRows(currentADItems);
  const totalsRow = buildTotalsRow(currentADItems);
...
<head>
<meta charset="utf-8" />
<title>Disposal Form ${escapeHtml(adNo)}</title>
<style>
  * { box-sizing: border-box; }
  ...
  /* ---------- Disposal type pill (in items table) ---------- */
  .type-pill {
    display: inline-block;
    padding: 1px 8px;
    border-radius: 8px;
  ...
  .type-sell { background: #e3f6e8; color: #1b7a3d; }
  .type-dispose { background: #fdeaea; color: #b3261e; }
  ...
  <div class="page">
    ...
    <div class="content">
      ...
      <div class="letterhead">
        ...
        <div class="doc-title">
          <h1>DISPOSAL FORM</h1>
          <div class="ad-no">No. ${escapeHtml(adNo) || '—'}</div>
        </div>
      </div>
      ...
      <table class="info-grid">
        ...
        <tr>
          ...
          <td class="value">${escapeHtml(evaluatedBy)}</td>
        </tr>
        <tr>
          <td class="label">Prepared By</td>
          <td class="value">${escapeHtml(userName || evaluatedBy)}</td>
      ...
      <div class="section-label">Disposal Items</div>
      <table class="items-table">
        <thead>
          <tr>
            <th style="width:3%">#</th>
            <th style="width:10%">Asset No.</th>
            <th style="width:15%">Asset Name</th>
            <th style="width:5%">Qty</th>
            <th style="width:5%">UOM</th>
            <th style="width:24%">Reason for Disposal</th>
            <th style="width:9%">Type</th>
            <th style="width:10%">Salvage Amount</th>
      ...
      <div class="signatures">
        <div class="sig-block">
          <div class="sig-line">${escapeHtml(evaluatedBy)}</div>
          <div class="sig-role">Evaluated By</div>
        </div>
        <div class="sig-block">
      ...
      <div class="print-footer">
        ...
        <span>Disposal Form No. ${escapeHtml(adNo) || '—'}</span>
      </div>
...
/**
 * Opens a new tab containing the printable Disposal Form and, once the
 ...
 */
export function openADPrintPreview(options = {}) {
  const html = generateADPrintHTML(options);

  const printWindow = window.open('', '_blank');
  if (!printWindow) {
    // Pop-up blocked
    // eslint-disable-next-line no-alert
    alert('Please allow pop-ups for this site to preview and print the Disposal Form.');
    return;
  }
...
export default openADPrintPreview;