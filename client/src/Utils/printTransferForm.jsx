// Utils/printTransferForm.js
//
// Builds a professional, print-ready Transfer Form (TR) document and opens
// it in a new browser tab/window so the user can preview and print (or
// "Save as PDF") it via the native browser print dialog.
//
// Mirrors Utils/printJobOrder.js so the two documents share the same
// letterhead, company info, and overall look & feel.
//
// Usage (see transferFormPage.jsx):
//
//   import { openTRPrintPreview } from '../../../Utils/printTransferForm';
//
//   <CustomBtn variant='printBtn' iconType='print' onClick={() => openTRPrintPreview({
//     currentHeader,
//     currentTRItems,
//     docStatus,
//     companyConfig,
//     userName,
//     totalLevels,
//     approvalLogs,      // optional - filtered approval log rows for this TR
//   })}>
//     Preview
//   </CustomBtn>
...
// Mirrors the docStatus() mapping used in transferFormPage.jsx, with a
...
function buildItemsRows(items) {
  if (!items || items.length === 0) {
    return `
      <tr>
        <td colspan="11" class="empty-row">No line items on this Transfer Form.</td>
      </tr>`;
  }

  return items
    .map((row, idx) => {
      const serialNo = row.serialNo ?? row.serial_no;
      return `
        <tr>
          ...
          <td class="center">${formatDate(row.Date_Aq)}</td>
          <td class="amount-cell">${formatAmount(row.Amount_aq)}</td>
...
export function generateTRPrintHTML({
  currentHeader = {},
  currentTRItems = [],
  ...
  // — kept identical to printJobOrder.js so both documents share letterhead info.
  ...
  const trNo = currentHeader?.TR_No || '';
  ...
  const transferTo = currentHeader?.Department || '—';
  const newHolder = currentHeader?.Holder || '—';
  const custodian = currentHeader?.Custodian || userName || '—';
  const location = currentHeader?.Location || '—';
  const remarks = currentHeader?.Remarks || '';

  const itemsRows = buildItemsRows(currentTRItems);
...
<head>
<meta charset="utf-8" />
<title>Transfer Form ${escapeHtml(trNo)}</title>
<style>
  * { box-sizing: border-box; }
  ...
  .num-cell { text-align: center; width: 22px; color: #777; }
  ...
  <div class="page">
    ...
    <div class="content">
      ...
      <div class="letterhead">
        ...
        <div class="doc-title">
          <h1>TRANSFER FORM</h1>
          <div class="tr-no">No. ${escapeHtml(trNo) || '—'}</div>
        </div>
      </div>
      ...
      <table class="info-grid">
        ...
        <tr>
          <td class="label">Transfer To (Dept.)</td>
          <td class="value">${escapeHtml(transferTo)}</td>
          <td class="label">New Holder</td>
          <td class="value">${escapeHtml(newHolder)}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td class="value">${escapeHtml(userName || custodian)}</td>
      ...
      <table class="items-table">
        <thead>
          <tr>
            ...
            <th style="width:9%">Acquired Date</th>
            <th style="width:9%">Acquired Cost</th>
      ...
      <div class="signatures">
        <div class="sig-block">
          ...
          <div class="sig-role">Released By</div>
        </div>
        <div class="sig-block">
        ...
        <div class="sig-block">
          <div class="sig-line">${escapeHtml(newHolder)}</div>
      ...
      <div class="print-footer">
        ...
        <span>Transfer Form No. ${escapeHtml(trNo) || '—'}</span>
      </div>
...
/**
 * Opens a new tab containing the printable Transfer Form and, once the
 ...
 */
export function openTRPrintPreview(options = {}) {
  const html = generateTRPrintHTML(options);

  const printWindow = window.open('', '_blank');
  if (!printWindow) {
    // Pop-up blocked
    // eslint-disable-next-line no-alert
    alert('Please allow pop-ups for this site to preview and print the Transfer Form.');
    return;
  }
...
export default openTRPrintPreview;