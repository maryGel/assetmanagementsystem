...
//
// Mirrors the layout/letterhead conventions of Utils/printJobOrder.js so
// that all printed documents in the system share the same company
// letterhead, typography, and page styling.
//
// Usage (see assetAccPage.jsx):
//
//   import { openAAPrintPreview } from '../../../Utils/printAssetAccountability';
//
//   <CustomBtn onClick={() => openAAPrintPreview({
//     currentHeader,
//     currentAAItems,
//     docStatus,
//     companyConfig,
//     userName,
//     totalLevels,
//     approvalLogs,      // optional - filtered approval log rows for this AA
//   })}>
//     Preview
//   </CustomBtn>
...
// Mirrors the docStatus() mapping used in assetAccPage.jsx, with a
...
function buildItemsRows(items) {
  if (!items || items.length === 0) {
    return `
      <tr>
        <td colspan="9" class="empty-row">No line items on this Asset Accountability.</td>
      </tr>`;
  }
...
function buildTotalRow(items) {
  ...
  return `
    <tr class="total-row">
      <td colspan="6" class="total-label">Total Acquisition Cost</td>
...
export function generateAAPrintHTML({
  currentHeader = {},
  currentAAItems = [],
  ...
  // (kept identical to printJobOrder.js so both documents share one letterhead).
  ...
  const aaNo = currentHeader?.AAFNo || '';
  ...
  const itemsRows = buildItemsRows(currentAAItems);
  const totalRow = buildTotalRow(currentAAItems);
...
<head>
<meta charset="utf-8" />
<title>Asset Accountability ${escapeHtml(aaNo)}</title>
<style>
...
<body>
  ...
  <div class="page">
    ...
    <div class="content">
      ...
      <div class="section-label">Accountable Assets</div>
      <table class="items-table">
        ...
        <tbody>
          ${itemsRows}
          ${currentAAItems && currentAAItems.length > 0 ? totalRow : ''}
        </tbody>
      </table>
      ...
      <div class="signatures">
        <div class="sig-block">
          ...
          <div class="sig-role">Prepared By / Custodian</div>
        </div>
        <div class="sig-block">
          <div class="sig-line">${escapeHtml(employeeName)}</div>
          <div class="sig-role">Received By / Accountable Employee</div>
        </div>
        <div class="sig-block">
      ...
      <div class="print-footer">
        ...
        <span>Asset Accountability No. ${escapeHtml(aaNo) || '—'}</span>
      </div>
 ...
 */
export function openAAPrintPreview(options = {}) {
  const html = generateAAPrintHTML(options);
...
export default openAAPrintPreview;